...
export function SupportChat({ orgId }: { orgId: string }){
  const [messages, setMessages] = useState<{role:'user'|'assistant'; content:string}[]>([{ role:'assistant', content:'Hi! I can answer questions and help you book.' }]);
  ...
  const send = async (formData: FormData) => {
    ...
    const res = await fetch('/api/chat/support', { method:'POST', body: JSON.stringify({ orgId, messages: [...messages, { role:'user', content:text }] }) });
    const data = await res.json().catch(()=>({ reply:'Demo reply' }));
    setMessages(m=>[...m, { role:'assistant', content: data.reply || 'Demo reply' }]);
  };
  return (<div className="rounded-2xl border border-surface-outline p-4 bg-surface-raised">
    ...
    <form ref={formRef} action={send} className="mt-3 relative">
      <input name="prompt" placeholder="Ask about services, hours, pricing, policies..." className="w-full rounded-bubble bg-surface border border-surface-outline px-4 py-3 pr-24 outline-none" />
